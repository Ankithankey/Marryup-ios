        ...
        <ScrollView showsVerticalScrollIndicator={false} indicatorStyle='white'>
          <View style={styles.cardcontainer}>
            {item.profilePictureUrl ? (
              <Image source={{ uri: item.profilePictureUrl || require('@/assets/images/addphoto.png') }} style={styles.profileImage} />
             
            ) : (
              <Image source={require('../../assets/images/noImage.png')} resizeMode='cover' style={{ width: 155 }} />
            )}
      ...
      {loading && currentPage === 1 ? (
        ...
        <FlatList
          ...
          <View style={{ justifyContent: 'center', alignItems: 'center', flex: 1 }}>
            <ActivityIndicator size={30} color="#A8D5BA" />
            <Text style={{ fontSize: 20, marginTop: 10, fontWeight: '300' }}> Please wait... </Text>
          </View>: null}
        />
      )}
  {/* {pagination} */}
                        ...
                        <View style={{ backgroundColor: 'white', padding: 15, width: '100%', borderRadius: 10, height: '100%', justifyContent: 'center', alignItems: 'center' }}>
                            {selectedUser && (
                                <>
                                    ...
                                    <Text style={styles.item}>Email: {selectedUser.email|| 'N/A'} </Text>
                                    <Text style={styles.item}>Address: {selectedUser.address|| 'N/A'} </Text>
                                    <Text style={styles.item}>State: {selectedUser.state|| 'N/A'} </Text>
                                    <Text style={styles.item}>Country: {selectedUser.country|| 'N/A'} </Text>
                                    <Text style={styles.item}>Phone: {selectedUser.phoneNumber|| 'N/A'} </Text>
                                </>
                            )}
